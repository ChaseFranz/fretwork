...
- **Dark or light**: the page follows your system's setting and the button at the top right switches it, remembered in your browser; the graph, the compare colours and the About page follow
...
2. `.env` in the repo root - copy `.env.example` and set `FRETWORK_BUCKET`, and `FRETWORK_DISTRIBUTION` if CloudFront is in front of it. `FRETWORK_INDEXNOW_KEY` (any 32 hex digits) makes every deploy tell Bing, Yandex and the engines that read them which pages changed, through [IndexNow](https://www.indexnow.org/); publish puts the key at the site's root as `<key>.txt`, which is how the engines know the request is yours, so it is not a secret. **No credentials go in this file**; `deploy.py` refuses one that has any.
...
```
python tools/check_site.py --site site/Local          # the thirteen live checks; the live strapline must match the one just published
...
**Search engines** (section 21 of the spec), once, by hand, since both need the domain's owner:

- Google Search Console: add `fretladder.com` as a domain property, verify it with the DNS TXT record it gives you (at the registrar, on the apex), then Sitemaps: submit `https://fretladder.com/sitemap.xml`. URL inspection on one song page (`https://fretladder.com/song/<key>.html`, any key from the sitemap) says whether Google sees a page or a redirect: it must be a page. Bing Webmaster Tools takes the same sitemap and can import the Google verification.
- `www.fretladder.com`: `bash tools/www_redirect.sh`, once, with the SSO session live. The domain is in Route 53 in the same account and the certificate already covers `*.fretladder.com`, so the script does the rest and changes only what differs: the CloudFront Function in `tools/cloudfront/www-to-apex.js` (created or updated from the file, tested on a `www` request, published), `www.fretladder.com` as the distribution's second alias with the function on its viewer request, the `www` A and AAAA alias records, a wait for the distribution, then `curl` from outside; `https://www.fretladder.com/?code=X` must answer `301` with a `location` on the apex, path and query kept. It is the maintainer's to run, like `deploy.py`.
- Crawlers may fetch `data/` on purpose: the rows are there, and a crawler that renders the page needs them or it indexes an empty table. `graph/` stays disallowed.
