...
- `www.fretladder.com` answers nothing today. To make it redirect to the apex: a CNAME (or an alias) for `www` at the registrar pointing at the distribution; the certificate in ACM (us-east-1) reissued or re-requested to cover `www.fretladder.com` as a second name, with its own DNS validation record; `www.fretladder.com` added to the distribution's alternate domain names; and a CloudFront Function on the viewer request that returns a 301 to `https://fretladder.com` + the request's URI when the `host` header is the `www` name. Check with `curl -sI https://www.fretladder.com/song/` which must answer `301` with a `location` on the apex.
