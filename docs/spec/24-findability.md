...
8. **Every source and list page links the others.** A "More" block under the table on source and list pages lists every source page and every list, so a crawler that lands on one reaches all 49 and the link equity spreads; song pages keep their game link and the library link (49 more links on 2,260 pages would be bloat).
