...
1. **The front page is a page without JavaScript.** `index.html` gains `<section id="static">` between the header and the table: an `h1` that says what the site is in the query's words ("Difficulty ratings for every Guitar Hero, Rock Band and Clone Hero chart"), a paragraph on how the ratings are made and what the site holds (the counts from the frames), the ten hardest Expert charts per instrument as a real table with each song linking its page (`frames.counts`'s `hardest`, which the library page already draws), and a link to every game page, every list and every document page. The brand in the header becomes a `<p>`, so the page has one `h1`. `main.js` removes the section at boot, before the first paint, so a visitor with JavaScript sees the app exactly as before; a crawler, a reader mode, and Bing see a page. The section is about 12 KB. Rejected: `<noscript>` (Google reads it, Bing's renderer and the JS-off case differ, and a section is simpler to test); a separate landing page (the home URL is the one every link will point at); prerendering the app's table (the rows are 1.4 MB).

2. **Titles and headings carry the game and the words.** Song page: `{song} by {artist}: {game} chart difficulty - Fretladder` for a chart matched to a game, `{song} by {artist}: Clone Hero chart difficulty - Fretladder` for a custom (`fact['release']` is the game or the literal `Custom`), which also tells the 110 duplicate titles apart (one pair remains: the same song, twice in one game's DLC). The `h1` becomes "{song} by {artist}" and a line under it names the game and the pack. The description opens with the question people type, "How hard is {song} by {artist} in {game}?", then the share lines. Game page: title `{game} song list ranked by difficulty - Fretladder`, the `h1` the title without the brand (the brand becomes a link above it on every document page, as on the song pages). The list intros gain a sentence saying the list is computed from the chart files, not voted, in the words "difficulty list" and "difficulty chart".
...
6. **Structured data a search feature reads.** `BreadcrumbList` on song, game and list pages (Home > Game > Song; Home > Lists > List); `ItemList` on the lists and the game pages (position, name, url for each entry, which is what a ranked list is); `WebSite` with a `SearchAction` on the front page (`/?q={search_term_string}`, which the app already reads); `Dataset` on the library page, which Google Dataset Search indexes and which is a channel of its own for "guitar hero difficulty data". `MusicRecording` stays.
...
- `labels.UI['song_page_title']` takes `{game}`; `song_page_title_custom`; `song_desc_lead` ("How hard is {song} by {artist} in {game}?"); `song_where` (the line under the h1); `game_title` becomes "{game} song list ranked by difficulty"; `list_method`; `more_games`, `more_lists`; `breadcrumb_home`, `breadcrumb_lists`.
...
- `bundle.write_page` returns the names it wrote; `publish()` returns them; `deploy.indexnow(key, host, urls, dry_run)`; `deploy.BUNDLE_TOP` admits `<32 hex>.txt`; `bundle.PAGE_TOP` likewise; `envfile` key `FRETWORK_INDEXNOW_KEY`.
...
- The static section must be removed before the app's first paint, not hidden by CSS alone, or the page has two tables for a moment and a screen reader reads both.
- A fragment link is client-side only: a visitor without JavaScript who clicks a number lands on the front page's static section, which is the right fallback.
