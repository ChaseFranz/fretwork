...
- Fixed 2026-09-13, found by the larger library (8,781 Guitar rows): a jump to the end painted a window short of it, since `paint` read the scroll position against the estimate the *new* spacers were about to stand at rather than the one the current spacers stood at, and the two differ. `paint` now maps the position with `state.window.avg`, builds the spacers at `state.window.next`, and then puts the scroller right itself: the held row back where it was after a scroll-driven paint, else the row the position asked for at the top (a long jump, and a sort at a changed estimate, which keeps the place in the list rather than the pixel); `.fw-wrap` has `overflow-anchor: none`, so the browser's own scroll anchoring does not add a correction on top.
- Fixed 2026-09-13, found by the larger library: the pane remembered the row that opened it as a DOM node, and focusing a row below the fold scrolls, which can repaint the window and replace the node, so Escape landed on the tab stop instead; the opener is remembered by code and revealed on close.
