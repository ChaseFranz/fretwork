...
**Status:** Landed (2026-09-12, 331e71e; awaiting the maintainer's deploy). Written 2026-09-12 against `main` at `e23f592`, from the code and the Local library (1,748 songs by `SongKey`, 11,904 charts). As landed: the pages are about 2.1 KB, not 900 bytes, since each carries the theme script and its own two palettes like the other document pages (the template's comment went to `page.py` so it is not sent 1,748 times); the description names the part (`Expert Lead: D 169.93, Calc Tier 8, at or above 97%`), not the sheet, since Guitar holds three parts; `song.html` is its own template; `deploy.plan` is nine commands with the song sync, the fixture's stub `aws` learned the folder and the xml type.
