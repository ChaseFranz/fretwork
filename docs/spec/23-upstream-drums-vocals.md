...
- Page suites: `graph.js` on the first sheet, Drums and Vocals (the legend is ~D and the family's lines, each swatch its token, the readout every line's word, the alt text the family's words, the geometric mean and the weighted sums through `smooth()`), `contrast.js` (the kicks token), `narrow.js` (six tables, 29 formulas; a table or formula wider than the screen scrolls in its own box and the body never does, since how many overflow at 390 px depends on the machine's fonts: CI's Linux Chrome widens the CalcTier table and five formulas, Windows Chrome three formulas).
