...
| 03 | [Pack registry and changelog page](03-changelog-packs.md) | Landed (2026-09-11, 15aea0b..229297c; tag fretladder-v1.2.0) | M | 00 | Committed `packs.toml` (26 seed packs, hand-written `[[change]]` entries), `functions/packs.py` (loader, validation, `append_pack`, folder join), an `Added` column, `changelog.html` on a shared `doc.html` template, the strapline linked to it, publish refusing an unregistered folder. |
