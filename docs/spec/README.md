...
| 14 | [Details pane and the link columns](14-details-pane.md) | Landed on main (2026-09-11 evening; the legend fix 9a4add5, the pane in the commit after) | L | 06, 07, 13 | The graph and the song grid become one details pane under the table, a region rather than two stacked dialogs: the row is highlighted, the arrows move the chart, the height drags, the caret collapses; `Enchor` and `Leaderboard` boolean columns after Artist with the arrow in the cell, the same links leading the pane's tool row; `?song=` an alias for the song's primary chart; a compare legend that names only what differs. |
