...
| 00 | Whether upstream takes the backup-header PR | The maintainer's answer; the fork carries the fix either way |
...
| 05, 06 | `Compress` on the CloudFront distribution | One `aws cloudfront get-distribution-config ... Compress` query with a live login; if `false`, one console checkbox before 05's step 9 deploy |
