...
- `pane.js` exports `openPane(code, vs, {follow, reveal})`, `closePane`, `paneIsOpen`, `addCompare`, `removeCompare`, `startPicking`, `stopPicking`, `refreshLinks`, `initPane`. `song.js` exports `chartsOf`, `folders`, `primaryCode`, `songSection`. `links.js` exports `CHART_COL`, `LB_COL`, `linkFor(songKey, kind)` (kind a host key or `lb`), `linkAnchors`, `linkCell`, `fillLinkCells`. `table.js` exports `holdRow`. `load.js` dispatches `fw:links`.
