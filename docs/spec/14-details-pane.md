...
10. **A row is one chart.** The first cut kept the extras across a row click (the row replaced A, B and C stayed), which the maintainer found impossible to track: A was both the row and the song on the right, and the lines that stayed were invisible in the table. Now a row click or an arrow key shows that chart alone; comparisons are built only in the pane (a cell, Compare all levels, Compare with a row), the open row clicked while comparing drops to that chart and clicked again closes, and `table.markRows` marks every row on the graph with its series colour on the left edge and its letter after the rank, so the table and the grid mirror the legend alike. The edge's fill shadow carries Bootstrap's own fallback chain, or an unhovered row's undefined `--bs-table-bg-state` voids the declaration.
11. **Compare is pick-from-the-table alone.** The pane's own picker (a search box over title, artist and code, with the song's other charts for an empty box) went the same evening, on the maintainer's call: the table's search and filters above are a better picker than a duplicate of them, and the song grid beside the graph already lists the song's other charts with a "Compare all levels" per instrument. One button, "Compare with a row", starts the picking; the bar above the table says what is waiting.
