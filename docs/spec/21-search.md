...
6. **The maintainer's part**, written into README's "After it is live": verify the domain in Google Search Console with a DNS TXT record and submit `sitemap.xml`; the same in Bing Webmaster Tools; `www`: the domain turned out to be in Route 53 in the same account and the certificate to cover `*.fretladder.com` already, so `tools/www_redirect.sh` does it all in one idempotent pass (the CloudFront Function in `tools/cloudfront/www-to-apex.js`, created, tested against a `www` request with a query, published; the second alias and the viewer-request association on the distribution; the `www` A and AAAA aliases; a wait and a `curl`). The function was created and tested from here (a 301 to `https://fretladder.com/song/abc.html?code=10145439XG&vs=1%2C2`, the apex passed through untouched); publishing it, updating the distribution and writing the zone are production changes the assistant's permissions refuse, so the script is the maintainer's to run.
