...
cells draw the arrow from the links file (`links.linkCell`, filled in place by
`fillLinkCells` on the `fw:links` event when the file lands after the rows), and the
accent buttons that lead the pane's tool row (`linkAnchors`). `tools/enchor_lookup.py` resolves each song against Chorus Encore offline (exact
