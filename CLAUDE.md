...
(`--fw-link`), the code column included. The graph's three curves
(`--fw-curve-d/nps/vps`) and the three compare series (`--fw-series-a/b/c`,
magenta, cyan, gold) are two separate trios, so no colour changes meaning when
